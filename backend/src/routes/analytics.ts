...
router.get('/dashboard',
  ...
  asyncHandler(async (req: AuthenticatedRequest, res) => {
    ...
    // Get QA history
    const qaHistory = await qaService.getQAHistory(req.user!.uid, 100);
    ...
    const filteredLessons = progress.completedLessons.filter(
      lesson => lesson.completedAt && lesson.completedAt >= startDate
    );

    const filteredQuestions = qaHistory.messages.filter(
      message => new Date(message.createdAt) >= startDate
    );
    ...
    const overview = {
      ...
      lessonsThisMonth: filteredLessons.filter(
        lesson => lesson.completedAt && 
        lesson.completedAt >= new Date(now.getTime() - 30 * 24 * 60 * 60 * 1000)
      ).length,
...
// Helper functions
function generateDailyActivity(lessons: any[], questions: any[], startDate: Date, endDate: Date) {
  ...
  for (let i = 0; i < days; i++) {
    ...
    const dayQuestions = questions.filter(question => 
      new Date(question.createdAt).toISOString().split('T')[0] === dateStr
    );
...
function generateSubjectBreakdown(lessons: any[]) {
  const subjects: { [key: string]: { lessonsCompleted: number; timeSpent: number; scores: number[] } } = {};
...
function generateDifficultyProgress(lessons: any[]) {
...
function generateLearningTrends(lessons: any[], startDate: Date, endDate: Date) {
